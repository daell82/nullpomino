package mu.nu.nullpo.game.subsystem.ai.util;
...
public class Ranks implements Serializable {
	...
	// Number of different orientations a piece can have (this is used to save
	// computing time)
	...
	public static final int[][] PIECES_LEFTMOSTS = { { 0, 2, 0, 1 }, // I
			{ 0, 1, 0, 0 }, // L
			{ 0, 0, 0, 0 }, // O
			{ 0, 1, 0, 0 }, // Z
			{ 0, 1, 0, 0 }, // T
	...
	public static final int[][] PIECES_RIGHTMOSTS = { { 3, 2, 3, 1 }, // I
			{ 2, 2, 2, 1 }, // L
			{ 1, 1, 1, 1 }, // O
			{ 2, 2, 2, 1 }, // Z
			{ 2, 2, 2, 1 }, // T
	...
	public static final int[][] PIECES_WIDTHS = { { 4, 1, 4, 1 }, // I
			{ 3, 2, 3, 2 }, // L
			{ 2, 2, 2, 2 }, // O
			{ 3, 2, 3, 2 }, // Z
			{ 3, 2, 3, 2 }, // T
	...
	public static final int[][][] PIECES_LOWESTS = { { { 1, 1, 1, 1 }, { 3 }, { 2, 2, 2, 2 }, { 3 } }, // I
	...
	public Ranks(Ranks rankFrom) {
		...
		ranksFrom = rankFrom;
		maxJump = ranksFrom.getMaxJump();
		base = 2 * maxJump + 1;

		stackWidth = ranksFrom.getStackWidth();
		surfaceWidth = stackWidth - 1;
		size = (int) Math.pow(2 * maxJump + 1d, stackWidth - 1d);
		ranks = new int[size];
		completion = 0;
		error = 0;
		maxError = 0;
	...
	public boolean surfaceAddPossible(int[] surfaceDecodedWork, int piece, int rotation, int x) {
		...
		if (x > 0) {
			...
			if (checkMaxJump(surfaceDecodedWork[x - 1])) {
				return false;
			}
		}
		...
		for (int x1 = 0; x1 < PIECES_WIDTHS[piece][rotation] - 1; x1++) {
			...
			if (checkMaxJump(surfaceDecodedWork[x + x1])) {
				return false;
			}
		}

		if (x < surfaceWidth - (PIECES_WIDTHS[piece][rotation] - 1)) {
			surfaceDecodedWork[x + PIECES_WIDTHS[piece][rotation]
					- 1] -= PIECES_HEIGHTS[piece][rotation][PIECES_WIDTHS[piece][rotation] - 1];
			if (checkMaxJump(surfaceDecodedWork[x + PIECES_WIDTHS[piece][rotation] - 1])) {
				return false;
			}
		}
		return addPossible;
	...
	private boolean checkMaxJump(int value) {
		return value > maxJump || value < -maxJump;
	}
	...
	private int getRankPieceRotation(int[] surface, int[] surfaceDecodedWork, int piece, int rotation) {
		...
		for (int x = 0; x < stackWidth - (PIECES_WIDTHS[piece][rotation] - 1); x++) {
			...
			if (x < surfaceWidth - (PIECES_WIDTHS[piece][rotation] - 1)) {
				surfaceDecodedWork[x + PIECES_WIDTHS[piece][rotation] - 1] = surface[x + PIECES_WIDTHS[piece][rotation]
						- 1];
			}
