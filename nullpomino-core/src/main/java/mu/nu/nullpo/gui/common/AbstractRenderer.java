...
public abstract class AbstractRenderer<T> extends EventReceiver<T> {
	...
	protected void drawBlock(int x, int y, int color, int skin, boolean bone, float darkness, float alpha, float scale,
		...
		if (skin >= resources.getImgBlockListSize()) {
			skin = 0;
		}
	...
	protected abstract void drawBlockSpecific(int x, int y, int sx, int sy, float darkness, float alpha,
			AbstractImage img);

	protected abstract void drawOutlineSpecific(int i, int j, int x, int y, int blksize, Block blk, int outlineType);

	protected abstract void drawFieldSpecific(int x, int y, int width, int viewHeight, int blksize, float scale,
			int outlineType);
}
