...
public class ComboRaceSeedSearch extends DummyAI {
	...
	protected static int[] stateScores = { 6, 7, 7, 6, 8, 3, 2, 9, 3, 4, 3, 1, 8, 4, 1, 3, 1, 1, 4, 3, 9, 2, 3, 8, 4, 8,
			3, 3 };
	...
	public static void main(String[] args) {
		...
		for (long seed = 0L; seed < Long.MAX_VALUE; seed++) {
			...
			if (result > bestResult) {
				...
				bestResult = result;
				System.out
						.println("New best result: seed = " + Long.toString(bestSeed, 16) + ", result = " + bestResult);
			}
		}
	}
	...
	public static int thinkMain(int state, int holdID, int depth) {
		...
		while (t != null) {
			bestPts = Math.max(bestPts, thinkMain(t.newField, holdID, depth + 1) + 1000);
			t = t.next;
		}
		...
		if (holdID == -1) {
			...
			while (t != null) {
				bestPts = Math.max(bestPts, thinkMain(t.newField, nextQueueIDs[depth], depth + 1) + 1000);
				t = t.next;
			}
		}
