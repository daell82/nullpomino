...
public class Nohoho extends DummyAI implements Runnable {
		...
		if (engine.nowPieceObject != null && engine.stat == GameEngine.Status.MOVE && delay >= getMoveDelay()
			...
			if (rotateDir != 0) {
				if (engine.ruleopt.rotateButtonAllowDouble && rotateDir == 2 && !ctrl.isPress(Controller.BUTTON_E)) {
					input |= Controller.BUTTON_BIT_E;
				} else if (engine.ruleopt.rotateButtonAllowReverse && !engine.ruleopt.rotateButtonDefaultRight
						&& rotateDir == 1) {
	...
	public int thinkMain(int x, int y, int rt, int rtOld, Field fld, Piece piece, int defcon) {
		...
		if (!piece.placeToField(x, y, rt, fld)) {
			if (DEBUG_ALL) {
				log.debug("End of thinkMain(" + x + ", " + y + ", " + rt + ", " + rtOld + ", fld, piece " + piece.id
						+ ", " + defcon + "). pts = MIN_VALUE (Cannot place piece)");
			}
		...
		if (defcon >= 4) {
			...
			if (maxX < 2) {
				if (DEBUG_ALL) {
					log.debug("End of thinkMain(" + x + ", " + y + ", " + rt + ", " + rtOld + ", fld, piece " + piece.id
							+ ", " + defcon + "). pts = MIN_VALUE (Invalid location/defcon combination)");
				}
				return Integer.MIN_VALUE;
			}
